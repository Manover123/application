...
const Report = () => {
  ...
  const putData = () => {
    ...
    for (let i = 0; i < report.length; i++) {
      value.push([
        report[i].name_product,
        report[i].qty,
        report[i].old_price,
        report[i].unit_product,
        report[i].totalPrice,
  ...
  const sumamont = () => {
    let sum = 0;
    for (const value of Object.values(report.map(val => val.totalPrice))) {
      sum += value;
      setSumAmount(sum);
    }
  };
    ...
    const RowDataReport = () => {
      report.map((v, i) => {
        mapObj =
          mapObj +
          `<tr>
            <td><span>${v.name_product}</span></td>
            <td><span>${v.qty}</span></td>
            <td><span>${v.old_price}</span></td>
            <td><span>${v.unit_product}</span></td>
            <td><span>${v.totalPrice}</span></td>
            <td><span>${format(
  ...
  useEffect(() => {
    console.log(memberData[0].id_us);
    putData();
    sumamont();
    createRow();
        ...
        <View
          ...
          <View style={{flexDirection: 'row', justifyContent: 'center'}}>
            <Text
              ...
              รายงาน
              <Ioniconscan name="filetext1" size={30} color="#fff" style={{}} />
            </Text>
          </View>
        </View>
      </View>
...
const stypes = StyleSheet.create({
  container: {flex: 0.8, padding: 20, backgroundColor: '#fff'},
